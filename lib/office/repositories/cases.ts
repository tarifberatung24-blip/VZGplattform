...
class SupabaseCaseRepository implements CaseRepository {
  ...
  async create(input: NewCase) { const { data, error } = await this.client.from('cases').insert({ ...input, owner_id: this.userId }).select('*').single(); return { data, error: error?.message ?? null } }
