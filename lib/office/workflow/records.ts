...
import { GroqDraftGenerator, DRAFT_PROMPT_VERSION } from '../ai/groq-draft-generator'
import { consumeAiQuota, groqCircuitBreaker } from '../ai/guards'
import { interviewWithGroq } from '../ai/interviewer'
import { approvalMatches } from './approval'
...
const missingForDraft = (facts: Array<{ key: string }>) => ['recipient', 'subject', 'request'].filter(key => !facts.some(fact => fact.key.toLowerCase() === key))
const localQuestions = (locale: Locale, missing: string[]) => missing.slice(0, 3).map(key => ({ bg: `Моля, посочете ${key}.`, de: `Bitte geben Sie ${key} an.`, ru: `Пожалуйста, укажите ${key}.`, pl: `Proszę podać ${key}.`, sr: `Molimo navedite ${key}.`, ro: `Vă rugăm să indicați ${key}.` }[locale]))
