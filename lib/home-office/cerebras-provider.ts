...
async function requestCerebras(messages: unknown[], model = process.env.CEREBRAS_MODEL || "qwen-3.8-27b") {
...
export async function analyzeBescheidText(text: string) {
...
export async function analyzeBescheidImage(dataUri: string) {
  return requestCerebras([
    { role: "system", content: systemPrompt },
    { role: "user", content: [
...
// Backwards-compatible alias for the existing assistant contract.
export const analyzeWithGroq = analyzeBescheidText
