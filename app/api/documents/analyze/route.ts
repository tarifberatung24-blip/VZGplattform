...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { documentId?: unknown; mode?: unknown }
    ...
    if (!["uploaded", "failed", "analysis_not_configured"].includes(document.processing_status)) return NextResponse.json({ code: "ANALYSIS_ALREADY_RUNNING" }, { status: 409 })
    const claimed = await supabase.from("documents").update({ processing_status: "awaiting_analysis" }).eq("id", document.id).eq("household_id", householdId).in("processing_status", ["uploaded", "failed", "analysis_not_configured"]).select("id").maybeSingle()
    ...
    try {
      ...
      if (!text) {
        await supabase.from("documents").update({ processing_status: "analysis_not_configured" }).eq("id", document.id).eq("household_id", householdId)
