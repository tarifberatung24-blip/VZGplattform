...
`docs/HORIZON_EXECUTION_CHECKLIST.md` is the operational progress board. It summarizes verified
completion, active blockers, safe next work, and the current agent queue. It does not override the
Master Map or Build Ledger.

To avoid merge conflicts, worker agents must **not** edit the checklist unless the task explicitly
assigns them as the checklist/control-plane writer. Instead, every worker handoff must include:
...
Only verified remote/CI/deployment facts may be promoted to completed status. A future phase may be
audited or researched read-only while another implementation phase is active, but it must not be
implemented early.
