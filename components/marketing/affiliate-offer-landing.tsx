...
import { ArrowRight, Building2, CheckCircle2, CircleDollarSign, FileSearch, Lightbulb, ShieldCheck } from "lucide-react"
...
const content: Record<AffiliateOfferId, { icon: typeof FileSearch; bg: OfferCopy; de: OfferCopy }> = {
  "business-insurance": {
    icon: Building2,
    bg: {
      eyebrow: "Firmenversicherung", title: "Застраховки за твоята фирма, подготвени ясно", intro: "Подготви данните за дейността си и виж кои видове покритие предлага партньорът и как се различават: Betriebshaftpflicht, Inhaltsversicherung и Rechtsschutz. Офертата идва от партньора.", benefits: ["Betriebshaftpflicht, Inhaltsversicherung и Rechtsschutz разграничени", "Бранш, правна форма и служители подготвени", "Заяви директно при партньора"], steps: ["Опиши дейността и правната форма", "Избери желаните покрития", "Продължи към партньора за оферта"], caution: "Цената и приемането зависят от данните за дейността и се потвърждават от застрахователя. HORIZON by VZG не е застраховател.", cta: "Продължи към партньора", unavailable: "Партньорската връзка се настройва. Използвай формата за контакт, за да те уведомим." },
    de: {
      eyebrow: "Firmenversicherung", title: "Versicherungen für dein Unternehmen, klar vorbereitet", intro: "Bereite deine Angaben zur Tätigkeit vor und sieh, welche Deckungsarten der Partner anbietet und wie sie sich unterscheiden: Betriebshaftpflicht, Inhaltsversicherung und Rechtsschutz. Das Angebot kommt vom Partner.", benefits: ["Betriebshaftpflicht, Inhaltsversicherung und Rechtsschutz unterschieden", "Branche, Rechtsform und Mitarbeitende vorbereitet", "Direkt beim Partner anfragen"], steps: ["Tätigkeit und Rechtsform beschreiben", "Gewünschte Deckungen wählen", "Beim Partner ein Angebot anfragen"], caution: "Preis und Annahme hängen von den Angaben zur Tätigkeit ab und werden vom Versicherer bestätigt. HORIZON by VZG ist kein Versicherer.", cta: "Weiter zum Partner", unavailable: "Der Partnerlink wird gerade eingerichtet. Über das Kontaktformular informieren wir dich, sobald er aktiv ist." },
  },
  schufa: {
    icon: FileSearch,
    bg: {
      eyebrow: "SCHUFA и кредитоспособност", title: "Провери данните си, преди да кандидатстваш", intro: "Виж каква информация за кредитоспособността ти е налична и избери подходящия вид справка за твоята цел — например наем или подготовка за кредит.", benefits: ["По-ясна подготовка за важни договори", "Разлика между безплатен преглед на данни и платени продукти", "Партньорска връзка само след одобрение"], steps: ["Избери вида справка", "Идентифицирай се при партньора", "Прегледай резултата и условията"], caution: "HORIZON by VZG не вижда твоя SCHUFA резултат и не може да го променя.", cta: "Провери SCHUFA при партньора", unavailable: "Партньорската връзка се настройва. Използвай формата за контакт, за да те уведомим." },
    de: {
      eyebrow: "SCHUFA & Bonität", title: "Prüfe deine Daten, bevor du einen Antrag stellst", intro: "Sieh nach, welche Bonitätsinformationen verfügbar sind, und wähle die passende Auskunft für dein Ziel — zum Beispiel Wohnungssuche oder Kreditvorbereitung.", benefits: ["Besser vorbereitet für wichtige Verträge", "Kostenlose Dateneinsicht und kostenpflichtige Produkte klar unterscheiden", "Partnerlink erst nach Freigabe"], steps: ["Art der Auskunft wählen", "Beim Partner identifizieren", "Ergebnis und Konditionen prüfen"], caution: "HORIZON by VZG sieht deinen SCHUFA-Score nicht und kann ihn nicht verändern.", cta: "SCHUFA beim Partner prüfen", unavailable: "Der Partnerlink wird gerade eingerichtet. Über das Kontaktformular informieren wir dich, sobald er aktiv ist." },
  },
  credit: {
  ...
  kfz: {
    icon: ShieldCheck,
    bg: {
      eyebrow: "Kfz застраховка", title: "Прегледай варианти за автомобилна застраховка", intro: "Подготви данните за автомобила и видовете покритие, след което продължи към партньора, за да сравниш неговите предложения.", benefits: ["Разбери Haftpflicht, Teilkasko и Vollkasko", "Сравни самоучастие и условия", "Виж оферти директно при партньора"], steps: ["Подготви HSN/TSN и SF клас", "Въведи данните при партньора", "Сравни покритие, цена и условия"], caution: "Цената и приемането зависят от данните в заявката и се потвърждават от застрахователя.", cta: "Сравни Kfz застраховки", unavailable: "Партньорската връзка се настройва. Използвай формата за контакт, за да те уведомим." },
    de: {
      eyebrow: "Kfz-Versicherung", title: "Kfz-Versicherungsoptionen ansehen", intro: "Bereite Fahrzeugdaten und Deckungsarten vor und geh anschließend zum Partner, um dessen Angebote zu vergleichen.", benefits: ["Haftpflicht, Teilkasko und Vollkasko verstehen", "Selbstbeteiligung und Bedingungen vergleichen", "Angebote direkt beim Partner ansehen"], steps: ["HSN/TSN und SF-Klasse bereithalten", "Daten beim Partner eingeben", "Schutz, Preis und Bedingungen vergleichen"], caution: "Preis und Annahme hängen von den Antragsdaten ab und werden vom Versicherer bestätigt.", cta: "Kfz-Versicherungen vergleichen", unavailable: "Der Partnerlink wird gerade eingerichtet. Über das Kontaktformular informieren wir dich, sobald er aktiv ist." },
  },
  energy: {
    icon: Lightbulb,
    bg: {
...
export function AffiliateOfferLanding({ offer, isOffered }: { offer: AffiliateOfferId; isOffered: boolean }) {
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><section className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-300">{copy.eyebrow}</p><div className="mt-5 grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-start"><div><h1 className="max-w-3xl text-balance text-4xl font-bold tracking-tight sm:text-6xl">{copy.title}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">{copy.intro}</p><div className="mt-9 flex flex-wrap gap-3">{isOffered ? <Button asChild size="lg" className="bg-blue-600 hover:bg-blue-500"><a href={goUrl}>{copy.cta}<ArrowRight data-icon="inline-end" /></a></Button> : <Button asChild size="lg" variant="secondary"><Link href="/uslugi">{locale === "bg" ? "Свържи се с нас" : "Kontakt aufnehmen"}<ArrowRight data-icon="inline-end" /></Link></Button>}<Link href="/produkte" className="inline-flex items-center px-4 text-sm font-semibold text-slate-300 hover:text-white">{locale === "bg" ? "Всички услуги" : "Alle Angebote"}</Link></div>{!isOffered && <p className="mt-4 max-w-xl text-sm leading-6 text-amber-200">{copy.unavailable}</p>}</div><aside className="rounded-3xl border border-slate-700 bg-slate-900 p-7 shadow-2xl"><span className="flex size-12 items-center justify-center rounded-2xl bg-blue-500/15 text-blue-300"><Icon className="size-6" /></span><h2 className="mt-6 text-xl font-semibold">{locale === "bg" ? "Как работи" : "So funktioniert es"}</h2><ol className="mt-5 space-y-4">{copy.steps.map((step, index) => <li key={step} className="flex gap-3 text-sm leading-6 text-slate-200"><span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-blue-500 text-xs font-bold text-white">{index + 1}</span>{step}</li>)}</ol></aside></div><div className="mt-14 grid gap-4 md:grid-cols-3">{copy.benefits.map((benefit) => <div key={benefit} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 text-sm leading-6 text-slate-200"><CheckCircle2 className="mb-3 size-5 text-blue-300" />{benefit}</div>)}</div><p className="mt-8 max-w-3xl border-l-2 border-amber-400 pl-4 text-sm leading-6 text-slate-300">{copy.caution}</p><p className="mt-5 text-xs text-slate-400">{affiliateDisclosureFor(locale)}</p></section></main>
}
