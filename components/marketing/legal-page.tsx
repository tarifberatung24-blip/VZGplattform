...
const content: Record<LegalPageType, { title: string; titleBg: string; intro: string; sections: Array<[string, string]> }> = {
  ...
  privacy: {
    ...
    sections: [
      ...
      ["Technische Dienstleister", "Für Hosting und die technische Bereitstellung werden die jeweils eingesetzten Deployment- und Infrastruktur-Dienstleister genutzt. Supabase verarbeitet Authentifizierungs-, Datenbank- und Speicherinformationen für den geschützten Bereich."],
      ["KI-Analyse und Anfragen", "Wenn Sie eine Analyse oder den Chat ausdrücklich nutzen, können die eingegebenen Inhalte zur Verarbeitung an Groq übermittelt werden. Eine Weiterleitung von Service-Anfragen an n8n ist derzeit nicht produktiv aktiviert. Vor einer Aktivierung werden die konkrete Hosting- und Auftragsverarbeitungssituation sowie die Aufbewahrungsfristen hier ergänzt."],
      ["Kontaktanfragen", "Wenn Sie das Kontaktformular nutzen, werden Name, E-Mail-Adresse und Nachricht zur Bearbeitung der Anfrage in der dafür vorgesehenen Lead-Datenbank gespeichert. Die Daten werden nicht für optionale Werbe- oder Reichweitenmessung verwendet."],
      ["Lokale Demo-Daten", "Der nicht authentifizierte Home-Office-Demobereich kann Dokumente, Analysen und Prüfstände im localStorage dieses Browsers speichern. Diese Demo-Daten werden nicht automatisch an den Server übertragen. Sie können sie über die Demo-Funktion oder durch Löschen der Browserdaten entfernen."],
      ["Partnerlinks", "Verlassen Sie HORIZON by VZG über einen Partnerlink, gelten die Datenschutzinformationen des jeweiligen Partners. HORIZON by VZG erhält keinen Zugriff auf den Vertrag, den Sie direkt beim Partner abschließen."],
      ["Cookies und Reichweitenmessung", "Für Anmeldung, Sitzungsverwaltung und Spracheinstellung können technisch notwendige Cookies verwendet werden. Aktuell werden keine optionalen Marketing-, Analyse- oder Affiliate-Tracking-Skripte geladen. Externe Inhalte oder Partnerseiten werden nur auf eigenen Seitenaufrufen oder nach einem bewussten Klick geladen. Vor Einführung zusätzlicher, nicht notwendiger Technologien wird diese Erklärung und gegebenenfalls die Einwilligungsverwaltung ergänzt."],
