...
export function WorkplaceActionCenter({ firstName, nextAction, reviewCount, documentCount, contractCount, reminderCount }: Props) {
  ...
  const intakeOptions = de
    ? [
        ...
        { href: "/documents", label: "Ich muss ein Dokument vorbereiten", icon: FilePlus2 },
  ...
  return (
    <section className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]" aria-labelledby="action-center-title">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 sm:p-6">
        <div className="flex items-center justify-between"><h2 className="font-semibold">{de ? "Mein Status" : "Моят статус"}</h2><Link href="/documents" className="text-xs font-semibold text-primary">{de ? "Dokumente öffnen" : "Отвори документите"}</Link></div>
        <dl className="mt-5 grid grid-cols-2 gap-3">
          <div className="rounded-xl bg-muted/50 p-3"><dt className="text-xs text-muted-foreground">{de ? "Zu prüfen" : "Чакащи преглед"}</dt><dd className="mt-1 text-xl font-semibold">{reviewCount}</dd></div>
          <div className="rounded-xl bg-muted/50 p-3"><dt className="text-xs text-muted-foreground">{de ? "Dokumente" : "Документи"}</dt><dd className="mt-1 text-xl font-semibold">{documentCount}</dd></div>
          <div className="rounded-xl bg-muted/50 p-3"><dt className="text-xs text-muted-foreground">{de ? "Verträge" : "Договори"}</dt><dd className="mt-1 text-xl font-semibold">{contractCount}</dd></div>
          <div className="rounded-xl bg-muted/50 p-3"><dt className="text-xs text-muted-foreground">{de ? "Fristen" : "Срокове"}</dt><dd className="mt-1 text-xl font-semibold">{reminderCount}</dd></div>
        </dl>
