...
function date(value: string | null, locale: "bg" | "de") {
  return value
    ? new Intl.DateTimeFormat(locale === "de" ? "de-DE" : "bg-BG", { day: "2-digit", month: "short" }).format(new Date(value))
    : locale === "de" ? "Keine Daten" : "Няма данни"
}
  ...
  return <main className="min-h-[calc(100dvh-5rem)] min-w-0 overflow-x-hidden bg-background px-4 py-5 text-foreground sm:px-6 sm:py-6 lg:px-8">
    <div className="mx-auto min-w-0 max-w-[1440px]">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,0.8fr)]">
        <article className="min-w-0 overflow-hidden rounded-md border border-border bg-card p-4 shadow-none sm:p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{de ? "Kostenübersicht" : "Разходен преглед"}</p><h2 className="mt-1 text-lg font-semibold">{de ? "Monatliche Kosten nach Vertrag" : "Месечни разходи по договори"}</h2></div><Button variant="ghost" size="icon" className="shrink-0" asChild><Link href="/vertraege" aria-label={de ? "Verträge öffnen" : "Отвори договорите"}><ChevronRight className="size-4" /></Link></Button></div><div className="mt-6 flex h-44 min-w-0 items-end gap-2 sm:gap-4">{(groups.length ? groups : [[missingData, 0]]).map(([category, amount], index) => <div key={category} className="flex min-w-0 flex-1 flex-col items-center gap-2"><span className="max-w-full truncate text-[10px] text-muted-foreground">{amount ? money(amount as number) : "—"}</span><div className="flex h-32 w-full items-end rounded-t-lg bg-muted"><div className="w-full rounded-t-lg bg-primary transition-all" style={{ height: `${amount ? Math.max(12, ((amount as number) / maxGroup) * 100) : 12}%`, opacity: 1 - index * 0.12 }} /></div><span className="max-w-full truncate text-[11px] text-muted-foreground">{String(category)}</span></div>)}</div><p className="mt-4 text-xs text-muted-foreground">{de ? "Ohne bestätigte Partnerdaten werden keine Einsparungen oder neuen Angebote geschätzt." : "Без потвърдени партньорски данни не се изчисляват спестявания или нови оферти."}</p></article>
        <article className="min-w-0 overflow-hidden rounded-md border border-border bg-card p-4 shadow-none sm:p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{de ? "Aufmerksamkeit" : "Внимание"}</p><h2 className="mt-1 text-lg font-semibold">{de ? "Prüfungsliste" : "Опашка за преглед"}</h2></div><Bell className="size-5 shrink-0 text-muted-foreground" /></div><div className="mt-5 space-y-3">{[...contracts.filter((item) => item.monthly_amount == null || item.review_status === "needs_review").slice(0, 3).map((item) => ({ label: item.title, detail: de ? "Bestätigung erforderlich" : "Нуждае се от потвърждение", href: "/vertraege" })), ...documents.slice(0, 2).map((item) => ({ label: item.original_filename, detail: item.processing_status ?? (de ? "Hochgeladen" : "Качен документ"), href: "/documents" }))].slice(0, 4).map((item) => <Link key={`${item.href}-${item.label}`} href={item.href} className="flex min-h-14 items-center gap-3 rounded-md border border-border p-3 transition hover:border-primary/40 hover:bg-primary/5"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-primary/5 text-primary"><FileText className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.label}</span><span className="block truncate text-xs text-muted-foreground">{item.detail}</span></span><ArrowUpRight className="size-4 shrink-0 text-muted-foreground" /></Link>)}{reviewCount + missingCosts === 0 && <div className="rounded-md border border-dashed border-border p-5 text-center text-sm text-muted-foreground"><CheckCircle2 className="mx-auto mb-2 size-5 text-success" />{de ? "Keine Prüfung nötig." : "Няма задачи за преглед."}</div>}</div></article>
      </section>

      <section className="mt-6 min-w-0 overflow-hidden rounded-md border border-border bg-card p-4 shadow-none sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{de ? "Aktuelle Verträge" : "Последни договори"}</p><h2 className="mt-1 text-lg font-semibold">{de ? "Deine erfassten Verträge" : "Записани ангажименти"}</h2></div><Button variant="outline" size="sm" className="w-full sm:w-auto" asChild><Link href="/vertraege">{de ? "Alle ansehen" : "Виж всички"}<ArrowUpRight className="ml-2 size-4" /></Link></Button></div><div className="mt-4 overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="pb-3 font-medium">{de ? "Vertrag" : "Договор"}</th><th className="pb-3 font-medium">{de ? "Anbieter" : "Доставчик"}</th><th className="pb-3 font-medium">{de ? "Monatlich" : "Месечно"}</th><th className="pb-3 font-medium">{de ? "Status" : "Статус"}</th><th className="pb-3 font-medium">{de ? "Ende" : "Край"}</th></tr></thead><tbody>{contracts.slice(0, 6).map((item) => <tr key={item.id} className="border-b border-border/70 last:border-0"><td className="py-3 font-medium">{item.title}</td><td className="py-3 text-muted-foreground">{item.provider_name || missingData}</td><td className="py-3">{item.monthly_amount == null ? missingData : money(Number(item.monthly_amount))}</td><td className="py-3"><span className="whitespace-nowrap rounded-md bg-primary/5 px-2 py-1 text-xs text-primary">{item.review_status === "confirmed" ? (de ? "Bestätigt" : "Потвърден") : (de ? "Prüfung" : "Преглед")}</span></td><td className="py-3 text-muted-foreground">{date(item.end_date, locale)}</td></tr>)}{contracts.length === 0 && <tr><td colSpan={5} className="py-8 text-center text-muted-foreground">{de ? "Noch keine Verträge erfasst." : "Все още няма записани договори."}</td></tr>}</tbody></table></div><p className="mt-2 text-[11px] text-muted-foreground sm:hidden">{de ? "Wische horizontal, um alle Spalten zu sehen." : "Плъзни хоризонтално, за да видиш всички колони."}</p></section>
    </div>
  </main>
}
