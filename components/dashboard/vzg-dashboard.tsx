...
export function VzgDashboard({ firstName, profile, contracts, documents, reviewCount, reminders }: VzgDashboardProps) {
  ...
  return <main className="min-h-[calc(100dvh-5rem)] bg-background px-4 py-6 text-foreground sm:px-6 lg:px-8">
    <div className="mx-auto max-w-[1440px]">
      <header className="flex flex-col justify-between gap-5 border-b border-border pb-6 xl:flex-row xl:items-center">
        ...
        <div className="flex flex-col gap-3 sm:flex-row"><div className="relative sm:w-72"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="h-10 bg-card pl-9" placeholder={de ? "Verträge durchsuchen…" : "Търсене в договори…"} /></div><Button asChild className="h-10"><Link href="/vertraege"><Plus className="mr-2 size-4" />{de ? "Vertrag hinzufügen" : "Добави договор"}</Link></Button></div>
      </header>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,0.8fr)]">
        <article className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{de ? "Cost overview" : "Разходен преглед"}</p><h2 className="mt-1 text-lg font-semibold">{de ? "Monthly costs by contract" : "Месечни разходи по договори"}</h2></div><Button variant="ghost" size="icon" asChild><Link href="/vertraege" aria-label="Open contracts"><ChevronRight className="size-4" /></Link></Button></div><div className="mt-6 flex h-44 items-end gap-2 sm:gap-4">{(groups.length ? groups : [["NEEDS_DATA", 0]]).map(([category, amount], index) => <div key={category} className="flex flex-1 flex-col items-center gap-2"><span className="text-[10px] text-muted-foreground">{amount ? money(amount as number) : "—"}</span><div className="flex h-32 w-full items-end rounded-t-lg bg-muted"><div className="w-full rounded-t-lg bg-primary transition-all" style={{ height: `${amount ? Math.max(12, ((amount as number) / maxGroup) * 100) : 12}%`, opacity: 1 - index * 0.12 }} /></div><span className="max-w-full truncate text-[11px] text-muted-foreground">{String(category)}</span></div>)}</div><p className="mt-4 text-xs text-muted-foreground">{de ? "No savings or new offers are estimated without verified partner data." : "Без потвърдени партньорски данни не се изчисляват спестявания или нови оферти."}</p></article>
        <article className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{de ? "Attention" : "Внимание"}</p><h2 className="mt-1 text-lg font-semibold">{de ? "Review queue" : "Опашка за преглед"}</h2></div><Bell className="size-5 text-muted-foreground" /></div><div className="mt-5 space-y-3">{[...contracts.filter((item) => item.monthly_amount == null || item.review_status === "needs_review").slice(0, 3).map((item) => ({ label: item.title, detail: de ? "Needs confirmation" : "Нуждае се от потвърждение", href: "/vertraege" })), ...documents.slice(0, 2).map((item) => ({ label: item.original_filename, detail: item.processing_status ?? "uploaded", href: "/documents" }))].slice(0, 4).map((item) => <Link key={`${item.href}-${item.label}`} href={item.href} className="flex items-center gap-3 rounded-xl border border-border p-3 transition hover:border-primary/40 hover:bg-primary/5"><span className="grid size-8 place-items-center rounded-lg bg-primary/10 text-primary"><FileText className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.label}</span><span className="block text-xs text-muted-foreground">{item.detail}</span></span><ArrowUpRight className="size-4 text-muted-foreground" /></Link>)}{reviewCount + missingCosts === 0 && <div className="rounded-xl border border-dashed border-border p-5 text-center text-sm text-muted-foreground"><CheckCircle2 className="mx-auto mb-2 size-5 text-success" />{de ? "Nothing needs review." : "Няма задачи за преглед."}</div>}</div></article>
      </section>

      <section className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{de ? "Recent contracts" : "Последни договори"}</p><h2 className="mt-1 text-lg font-semibold">{de ? "Your recorded commitments" : "Записани ангажименти"}</h2></div><Button variant="outline" size="sm" asChild><Link href="/vertraege">{de ? "View all" : "Виж всички"}<ArrowUpRight className="ml-2 size-4" /></Link></Button></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-border text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="pb-3 font-medium">{de ? "Contract" : "Договор"}</th><th className="pb-3 font-medium">{de ? "Provider" : "Доставчик"}</th><th className="pb-3 font-medium">{de ? "Monthly" : "Месечно"}</th><th className="pb-3 font-medium">{de ? "Status" : "Статус"}</th><th className="pb-3 font-medium">{de ? "End" : "Край"}</th></tr></thead><tbody>{contracts.slice(0, 6).map((item) => <tr key={item.id} className="border-b border-border/70 last:border-0"><td className="py-3 font-medium">{item.title}</td><td className="py-3 text-muted-foreground">{item.provider_name || "NEEDS_DATA"}</td><td className="py-3">{item.monthly_amount == null ? "NEEDS_DATA" : money(Number(item.monthly_amount))}</td><td className="py-3"><span className="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">{item.review_status === "confirmed" ? (de ? "Confirmed" : "Потвърден") : (de ? "Review" : "Преглед")}</span></td><td className="py-3 text-muted-foreground">{date(item.end_date)}</td></tr>)}{contracts.length === 0 && <tr><td colSpan={5} className="py-8 text-center text-muted-foreground">{de ? "No contracts recorded yet." : "Все още няма записани договори."}</td></tr>}</tbody></table></div></section>
    </div>
  </main>
}
