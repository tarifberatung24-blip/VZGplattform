...
export function VzgDashboard({ firstName, profile, contracts, documents, reviewCount, reminders, selectedModule }: VzgDashboardProps) {
  ...
  return <main className="min-h-[calc(100dvh-3.5rem)] min-w-0 overflow-x-hidden bg-background px-4 py-5 text-foreground sm:px-6 sm:py-7 lg:px-8">
    <div className="mx-auto min-w-0 max-w-[1440px]">
      <header className="flex flex-col justify-between gap-5 border-b border-border pb-5 xl:flex-row xl:items-end">
        <div><p className="text-xs text-muted-foreground">VZGplattform <span className="mx-1">/</span> {de ? "Overview" : "Преглед"}</p><div className="mt-3 flex items-center gap-3"><div className="grid size-9 place-items-center rounded-lg bg-primary text-primary-foreground"><LayoutDashboard className="size-4" /></div><div><h1 className="text-2xl font-semibold tracking-tight">{de ? `Willkommen${firstName ? `, ${firstName}` : ""}` : `Добре дошъл${firstName ? `, ${firstName}` : ""}`}</h1><p className="mt-0.5 text-sm text-muted-foreground">{de ? "Finanzielle Übersicht aus bestätigten Daten." : "Финансов преглед от потвърдени данни."}</p></div></div></div>
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row"><div className="relative w-full sm:w-64"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="h-10 w-full bg-card pl-9 text-sm" placeholder={de ? "Verträge durchsuchen…" : "Търсене в договори…"} /></div><Button asChild className="h-10 w-full sm:w-auto"><Link href="/vertraege"><Plus className="mr-2 size-4" />{de ? "Vertrag hinzufügen" : "Добави договор"}</Link></Button></div>
      </header>
