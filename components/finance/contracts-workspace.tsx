...
export function ContractsWorkspace({ householdId, initialContracts, loadError }: { householdId: string; initialContracts: Contract[]; loadError?: string | null }) {
  ...
  return <section className="kintex-panel mt-8 space-y-6 p-6">
    ...
    <div className="space-y-2">{contracts.length === 0 ? <p className="bg-secondary p-4 text-sm text-muted-foreground">{copy.noContracts}</p> : contracts.map((contract) => <div key={contract.id} className="flex items-center justify-between border border-border p-4"><div><p className="font-medium">{contract.title}</p><p className="text-sm text-muted-foreground">{contract.provider ?? copy.categories[contract.category]}</p></div><div className="text-right"><p className="font-semibold">{contract.monthly_cost == null ? "NEEDS_DATA" : `${Number(contract.monthly_cost).toFixed(2)} ${copy.monthlyUnit}`}</p><p className="text-xs text-muted-foreground">{contract.review_status === "confirmed" ? copy.confirmed : copy.reviewOpen}</p></div></div>)}</div>
