...
export function ContractsWorkspace({ householdId, initialContracts, loadError }: { householdId: string; initialContracts: Contract[]; loadError?: string | null }) {
  ...
  async function addContract() {
    ...
    if (error) setMessage("Vertrag konnte nicht gespeichert werden.")
      ...
      setMessage("Gespeichert. VZG Radar nutzt diesen Vertrag ab sofort als gespeicherte Quelle.")
    }
    setSaving(false)
  }
  ...
  return <section className="mt-8 space-y-6 border border-border bg-card p-6">
    ...
    <div className="flex items-start gap-3 border border-primary/30 bg-primary/5 p-4 text-sm text-muted-foreground"><Radar className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden="true" /><p>VZG Radar liest nur gespeicherte Verträge. Ohne Beträge oder Dokumente wird keine Ersparnis berechnet.</p></div>
