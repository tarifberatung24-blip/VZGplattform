...
import { ArrowUpRight, Bell, BookOpen, CheckCircle2, CircleDollarSign, FileText, Landmark, Receipt, ShieldCheck, WalletCards } from "lucide-react"
import { Button } from "@/components/ui/button"
import { createClient } from "@/lib/supabase/client"
import { useLanguage } from "@/lib/i18n/language-context"

type Props = { userId: string; firstName?: string | null; initialDeadlines: Array<{ id: string; title: string; due_at: string | null }> }

const modules = [
  { href: "/zayavka", label: "Angebotsdesk", description: "Anfrage stellen und manuell bearbeiten lassen", icon: FileText, tone: "bg-primary/10 text-primary" },
...
export function DashboardWorkspace({ userId, firstName, initialDeadlines }: Props) {
  ...
  async function addReminder() {
    ...
    const { data } = await supabase.from("deadlines").insert({ user_id: userId, title: "Finanzprofil vervollständigen", source: "dashboard" }).select("id,title,due_at").single()
    if (data) setDeadlines((current) => [...current, data])
    setSaving(false)
  }
    ...
    <main className="min-h-screen bg-background">
      <header className="border-b border-border bg-card/90">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5">
          <Link href="/" className="text-lg font-bold tracking-tight text-primary">HORIZON by VZG</Link>
          <div className="flex items-center gap-2"><Button asChild variant="ghost" size="sm"><Link href="/security"><ShieldCheck className="size-4" /> {de ? "Sicherheit" : "Сигурност"}</Link></Button><form action="/auth/logout" method="post"><Button variant="outline" size="sm">{de ? "Abmelden" : "Изход"}</Button></form></div>
        </div>
      </header>
      <div className="mx-auto max-w-6xl px-5 py-10">
        <section className="flex flex-col justify-between gap-6 border-b border-border pb-8 md:flex-row md:items-end" aria-labelledby="dashboard-title">
          <div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">Übersicht</p><h1 id="dashboard-title" className="mt-3 text-4xl font-bold tracking-tight text-foreground">Hallo{firstName ? `, ${firstName}` : ""}.</h1><p className="mt-3 max-w-xl text-base leading-7 text-muted-foreground">Baue Schritt für Schritt ein klares Bild deiner Finanzen in Deutschland auf.</p></div>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1.35fr_1fr]">
          ...
          <div className="rounded-2xl border border-border bg-card p-6"><div className="flex items-center justify-between"><h2 className="font-semibold text-foreground">Offene Erinnerungen</h2><Bell className="h-5 w-5 text-muted-foreground" /></div>{deadlines.length ? <ul className="mt-4 divide-y divide-border">{deadlines.slice(0, 3).map((item) => <li key={item.id} className="flex items-center gap-3 py-3 text-sm"><CheckCircle2 className="h-4 w-4 shrink-0 text-success" /><span className="flex-1 text-foreground">{item.title}</span><span className="text-xs text-muted-foreground">{item.due_at ? new Date(item.due_at).toLocaleDateString("de-DE") : "Offen"}</span></li>)}</ul> : <p className="mt-4 text-sm leading-6 text-muted-foreground">Noch keine offenen Aufgaben.</p>}<Button variant="ghost" size="sm" className="mt-2 px-0 text-primary hover:bg-transparent hover:text-primary" onClick={addReminder} disabled={saving}>{saving ? "Wird gespeichert …" : "Erinnerung hinzufügen"}</Button></div>
        </section>
      </div>
    </main>
  )
