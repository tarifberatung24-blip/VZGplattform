...
import { activeVzgModule, isVzgWorkspacePath, vzgModules } from "@/lib/vzg-navigation"
...
export function WorkspaceShell({ children }: { children: ReactNode }) {
  ...
  const active = activeVzgModule(pathname, search.get("module"))
  const de = locale === "de"
  const text = de
  ...
  if (!isVzgWorkspacePath(pathname)) return <><SiteHeader />{children}</>

  return (
    <div className="vzg-workspace min-h-screen bg-slate-50 text-foreground">
      <a href="#workspace-content" className="vzg-skip">{text.skip}</a>
      <header className="sticky top-0 z-40 flex h-20 items-center justify-between gap-3 border-b border-border bg-card/95 px-5 backdrop-blur lg:px-8">
        <Link href={localizedPath("/protected", locale)} aria-label="VZGplattform — BY VZG CONSULT" className="shrink-0">
        ...
        <aside id="workspace-navigation" className={cn("border-b border-border bg-card lg:sticky lg:top-20 lg:block lg:h-[calc(100dvh-5rem)] lg:overflow-y-auto lg:border-b-0 lg:border-r", open ? "block" : "hidden")}
          ...
          <div className="flex min-h-full flex-col px-4 py-6">
            ...
            <div className="mt-auto px-4 pt-8">
              ...
              <p className="mt-4 border-t border-border pt-4 text-xs text-muted-foreground">VZGplattform <span className="mx-1" aria-hidden="true">/</span> {text.pilot}</p>
            </div>
          </div>
        </aside>
