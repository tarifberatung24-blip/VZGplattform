...
export function TariffWorkspace() {
  return <section className="mt-8 rounded-3xl border border-border bg-card p-6 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Tarife</p><h2 className="mt-2 text-2xl font-semibold text-foreground">Проверка на реални оферти</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Партньорските предложения се отварят във външен доставчик. VZGplattform не измисля тарифи и не обещава спестяване.</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><Button asChild><Link href="/produkte">Отвори сравненията <ArrowRight data-icon="inline-end" /></Link></Button><Button asChild variant="outline"><a href="https://www.check24.de/internet/" target="_blank" rel="sponsored noopener noreferrer">Сравни интернет <ExternalLink data-icon="inline-end" /></a></Button></div><p className="mt-4 text-xs text-muted-foreground">Статус: EXTERNAL_PROVIDER · ако widget-ът не зареди, използвай директната връзка.</p></section>
}
