...
export function ProfileForm({ initialProfile }: { initialProfile: Profile }) {
  ...
  async function save(event: React.FormEvent) { event.preventDefault(); const response = await fetch("/api/profile", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ employment_status: form.employment_status || null, household_size: form.household_size ? Number(form.household_size) : null, monthly_income: form.monthly_income ? Number(form.monthly_income) : null, monthly_fixed_costs: form.monthly_fixed_costs ? Number(form.monthly_fixed_costs) : null }) }); setMessage(response.ok ? copy.saved : copy.saveError) }
